    ...
    public function seachAction($chaine) {
        ...
        if($valeur){
            foreach ($valeur as $key => $value) {
                $Valeur[$key] = $gestion->check($motclebase, $value)[0];
                foreach ($Valeur[$key] as $k => $value) {
                    $fin[$key][$k] = $Gmotclef->getfield($value);
                }
            }
                $session = $this->get('session');
                $session->set("key",true);
                $session->set('calculate', $fin);
         }else{
              $session = $this->get('session');
        ...
        if ($val == 1) {
            ...
            $date = [];
            isset($keywords[1]) ? $date = preg_split("/[-]+/", $keywords[1]) : $date = [];
    ...
    public function redir($date, $pays, $motcle) {
        ...
        switch ($a) {
            ...
            case 2:
                ...
                return $this->redirect($this->generateUrl('heebaridata_crossseach', ["pays1" => $pays[0]->getCountryName(), "pays2" => $pays[1]->getCountryName(), 'cle' => '?']));
                break;
            case 3:
                $gestion = $this->container->get('Heebari_data.gestion');
                $session = $this->get('session');
                ...
                $session->set('pays', $pays);
                $session->set('motcles', $motcle);
                return $this->redirect($this->generateUrl('heebaridata_crossseach2', ["pays1" => $pays[0]->getCountryName(), "pays2" => $pays[1]->getCountryName(), "pays3" => $pays[2]->getCountryName(), 'cle' => '?']));
                break;
            default :
                return $this->render('HeebaridataBundle:index:layout.html.twig');
                break;
